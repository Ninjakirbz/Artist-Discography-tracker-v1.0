...
def PrintArtists(ArtistList):                                               #this function prints the artist list
    for loop in range(len(ArtistList)):
        ...
        if ArtistList[0][1] == "PLACEHOLDER":
            print("You must add an album first")
            AddArtistCounter = -1
...
def DeletePlaceholder(ArtistList, Number):
    ArtistNum = PickArtist()
    TotalAlbums = ArtistList[ArtistNum][2]
    HeardAlbums = ArtistList[ArtistNum][3]              #updates the amount of albums an artist has
    ArtistName = ArtistList[ArtistNum][1]
...
while MainLoop == True:
    ...
    if AddArtistCounter == -1:
        AddArtistCounter = 0
        AddArtist(ArtistList, AddArtistCounter)
        DeletePlaceholder(ArtistList, AddArtistCounter)
        print("Now restart program")
